# evalpro — Employee Evaluation Management App
...
```bash
mysql -u root -p < eval-app/sql/schema.sql
```
...
Produces `eval-app/target/evaluation-app.war`.

## 4. Deploy

Copy `eval-app/target/evaluation-app.war` to your Tomcat `webapps/` directory and restart Tomcat.
